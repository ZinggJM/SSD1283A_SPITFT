...
#include "SSD1283A_SPITFT.h"

SSD1283A_SPITFT::SSD1283A_SPITFT(int8_t cs_pin, int8_t dc_pin, int8_t rst_pin, int8_t bl_pin) :
#if defined(ESP8266)
...
SSD1283A_SPITFT::SSD1283A_SPITFT(int8_t cs_pin, int8_t dc_pin, int8_t mosi_pin, int8_t sclk_pin, int8_t rst_pin, int8_t bl_pin) :
...
SSD1283A_SPITFT::SSD1283A_SPITFT(uint16_t width, uint16_t height,
...
void SSD1283A_SPITFT::begin(uint32_t freq)
{
  init(freq);
}

void SSD1283A_SPITFT::init(uint32_t freq)
{
...
void SSD1283A_SPITFT::setRotation(uint8_t r)
{
  rotation = r & 3;
...
void SSD1283A_SPITFT::invertDisplay(boolean i)
{
...
void SSD1283A_SPITFT::invert(boolean i)
{
  invertDisplay(i);
}

void SSD1283A_SPITFT::setAddrWindow(uint16_t x, uint16_t y, uint16_t w, uint16_t h)
{
  uint16_t x2 = x + w - 1;
  uint16_t y2 = y + h - 1;
  switch (rotation)
...
void SSD1283A_SPITFT::enableDisplay(boolean enable)
{
  if (_bl_pin >= 0) digitalWrite(_bl_pin, enable ? HIGH : LOW);
}

void SSD1283A_SPITFT::_writeCommandData16(uint8_t cmd, uint16_t data)
{
  writeCommand(cmd);
  SPI_WRITE16(data);
}

void SSD1283A_SPITFT::_init_table16(const void *table, int16_t size)
{
  static const uint16_t TFTLCD_DELAY16 = 0xFFFF;
